...
sys.path.append(str(PROJ_ROOT) + '../../GMM')
...
def run_mix_model(id=None, model=None):
    ...
    import torch 
    sys.path.append("..")
    from mix.model import SiameseDecider, SiameseDeciderOld
    from contrastive.mix.dataset import get_train_and_val_loader
   
    prefix = str(PROJ_ROOT) + "bing_maps/global-scrape/"
    file = str(PROJ_ROOT) + "turk/experiments/annotations.csv"
    save_file = str(PROJ_ROOT) + 'MODEL/siamese_experiments/initial.csv'
    img_file = str(PROJ_ROOT) + 'bing_maps/global-scrape/tile-list.txt'
    
    if model is None:
        weight_path = '../mix/weights/final4.ckpt'
